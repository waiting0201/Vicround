...
/**
 * 技術文章內頁 —— 版型對照 `mockup/Rounded Design/article.dc.html`（左側目錄）。
 *
 * <p>
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!article || !article.path.startsWith('/blog/')) return {};
...
export default async function Page({ params }: Params) {
  ...
  const article = await getArticle(locale, slug);
  if (!article || !article.path.startsWith('/blog/')) notFound();

  const more = await getArticles(locale, { type: 'technicalArticle', pageSize: 4 });
