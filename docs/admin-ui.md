...
- **側欄分區可收合（accordion）**：六個分區的標題本身是一顆按鈕，按了把整段收起來。
  多數編輯者一天只用其中兩三個分區，收掉其餘的，常用入口就不必每次在 25 項裡面找。
  四個細節是刻意的：
  - **各自獨立開合，不是「一次只能開一個」**——編輯者常在「內容」與「營運」之間
    來回，開一個就自動關掉另一個，等於每切一次頁都要多按一次。
  - **第一次進後台只開第一個分區**（「內容」，也是 `HOME_PATH` 的所在地），其餘收起來；
    之後照使用者自己的安排走——「我上次把它打開了，回來又被關上」比預設值不合口味
    惱人得多。
  - `localStorage` 記的是**被收起來的那幾個**，不是展開的那幾個。之後在 `menu.ts`
    新增分區時，記展開清單會讓新分區對所有舊使用者都是收起來的——新功能上線第一天
    沒有人看得到入口。（預設值改過時要換鍵名：沿用舊鍵的話，已經在用的人存的是舊
    預設的結果，新的預設對他們永遠不會生效。）
  - **跳進一個已收起的分區時自動展開它**（麵包屑、直接打網址、頁面內的連結）；
    但使用者自己收合正在看的那一區時不會被彈開，改在標題旁點一顆品牌色小點，
    讓「我在哪」這個訊號不會整個消失。
  - 第一個分區標題與頂欄分隔線之間留 20px（`nav` 的 `pt-5`，與分區之間的 `gap-5`
    同值）：第一個分區的上緣看起來就跟其他分區一樣是「一段的開頭」，而不是貼著
    那條線、像那條線的標籤。
  - **側欄收合成 64px 時分區一律全開**：那個狀態下沒有分區標題，也就沒有東西可以按，
    收著等於有一半圖示無故消失、而畫面上找不到把它們叫回來的地方。收起的分區同時
    掛 `inert`——只有高度 0 的話，鍵盤使用者仍然會 tab 進看不見的連結。
...
- **動態節奏**：`ds/tokens/shadows.css` 定義了 `--duration-*`／`--ease-*`，
  ...
  `.admin-accordion`（側欄分區收合），統一借用 ds 既有的節奏，不再是兩套時間曲線
  混用。全部都包了 `prefers-reduced-motion: reduce` 的退場開關。
  - `.admin-accordion` 用 `grid-template-rows: 0fr → 1fr` 做高度過場，不必量內容高度，
    也不必像 `max-height` 那樣猜一個數字：猜小了分區多幾項就被裁掉，猜大了動畫尾巴
    是一段什麼都沒發生的空轉。代價是 `overflow: hidden` 會連 focus ring 一起裁掉，
    所以側欄連結的 ring 改畫在框線內側（`outline-offset: -2px`）。
...
- 危險動作不要用委婉語：「刪除」不要寫成「移除」再讓使用者猜這是不是真的刪掉。
  刪除**就是真的從資料庫刪掉**（2026-09-12 起；原本是 `Archived` 軟刪除），不可復原，
  所以它用實心的 `danger` 鈕，而且一定要有確認框把後果講清楚（見 8.2）。
...
- 刪除產品：標題「刪除「Anti-Fog Film」」，說明「這個產品會從資料庫永久移除，不能復原；
  原本的網址會寫成 410（永久移除）。若還有其他內容引用它，刪除會被擋下來。」
